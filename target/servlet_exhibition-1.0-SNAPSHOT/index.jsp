...
<h1 class="text-body"><fmt:message key="common.hello"/><demo:Hello user="${user}"/>!</h1>
        ...
        <div class="col-9">
            <div class="row">
                <c:forEach items="${exhibitions}" var="product">
                    <div class="col-4">
                        <p><c:out value="${product.cost}"/></p>
                        <p><c:out value="${product.name}"/></p>
                        <p><c:out value="${product.subject}"/></p>
                        <p><input type="date" value="${product.getFormattedBeginning()}" readonly="readonly"></p>
                        <p><input type="date" value="${product.getFormattedEnd()}" readonly="readonly"></p>
                        <c:if test="${user.isPresent() && user.get().role.ordinal() > 0 && !user.get().hasExhibition(product.id)}">
                            <p>
                                <a href="${pageContext.request.contextPath}/exhibitions/auth/enroll?exhibitionId=${product.id}"><fmt:message
                                        key="common.enroll"/></a></p>
                        </c:if>
                        <c:if test="${user.isPresent() && user.get().role.ordinal() > 0 && user.get().hasExhibition(product.id)}">
                            <p><fmt:message key="common.alreadyEnrolled"/></p>
                        </c:if>
                    </div>
                </c:forEach>
            </div>
